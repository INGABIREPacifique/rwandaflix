...
import {
  BarChart3, Bell, Check, ChevronLeft, ChevronRight, Clock3, Download,
  ...
  return (
    <div className="app">
      <nav className="navbar">
        <button className="mobile-menu" onClick={() => setMobileOpen(v => !v)} aria-label="Open menu"><Menu size={23} /></button>
        ...
        <div className={`nav-links ${mobileOpen ? 'open' : ''}`}>
          ...
          <button onClick={() => { goTo('browse'); setActiveGenre('Drama') }}>Genres</button>
        ...
        <div className="nav-right">
          <label className="search-box"><Search size={16}/><input value={query} onChange={e => { setQuery(e.target.value); if (e.target.value) setActivePage('browse') }} placeholder="Search RwandaFlix" /></label>
          <div className="notification-wrap">
            <button className="icon-btn" onClick={() => setNotifications(v => !v)} aria-label="Notifications"><Bell size={18}/><i /></button>
      ...
      {selected && <div className="modal" onClick={e => e.target === e.currentTarget && setSelected(null)}><div className="modal-box"><button className="close" onClick={() => setSelected(null)}><X/></button><img className="modal-image" src={selected.image} alt=""/><div className="modal-content"><div className="eyebrow">RwandaFlix</div><h2>{selected.title}</h2><div className="hero-meta"><span>{selected.year}</span><span className="age">HD</span><span>{selected.genre}</span><span>{selected.duration}</span></div><p>{selected.description}</p><div className="detail-tags"><span>🇷🇼 Rwanda</span><span>98% Match</span><span>Subtitles</span></div><div className="hero-actions"><Button className="primary" onClick={() => openPlayer(selected)}><Play size={18} fill="currentColor"/> Play</Button><Button className="secondary" onClick={() => toggleList(selected)}>{list.has(selected.id) ? <Check size={18}/> : <Plus size={18}/>} {list.has(selected.id) ? 'In My List' : 'My List'}</Button></div></div></div></div>}

      {player && <div className="modal" onClick={e => e.target === e.currentTarget && setPlayer(null)}><div className="video-box"><button className="close" onClick={() => setPlayer(null)}><X/></button><div className="video-screen"><div className="player-brand">RWANDA<span>FLIX</span></div><div className="player-center"><button className="big-play" onClick={() => toast('Demo player ready — real streaming comes with the backend')}><Play size={34} fill="currentColor"/></button><h3>{player.title}</h3><p>Real HLS video streaming will be connected in the backend phase.</p></div></div><div className="video-controls"><span>▶</span><div className="progress"><i /></div><span>🔊</span><span>CC</span><span>⚙</span><span>⛶</span></div></div></div>}

      {login && <div className="modal" onClick={e => e.target === e.currentTarget && setLogin(false)}><div className="auth-box"><button className="close" onClick={() => setLogin(false)}><X/></button><div className="auth-logo">RWANDA<span>FLIX</span></div><h2>Welcome back</h2><p>Sign in to continue watching RwandaFlix.</p><input placeholder="Email address" type="email"/><input placeholder="Password" type="password"/><Button className="primary full" onClick={() => { setLogin(false); toast('Demo login successful 👋') }}>Sign In</Button><div className="or"><span>or</span></div><Button className="secondary full" onClick={() => { setLogin(false); toast('Demo account created 👋') }}>Create an account</Button><small>Prototype only — authentication will connect to Supabase in the backend phase.</small></div></div>}

      {dashboard && <div className="modal" onClick={e => e.target === e.currentTarget && setDashboard(false)}><div className="dashboard-box"><button className="close" onClick={() => setDashboard(false)}><X/></button><aside><div className="studio-logo"><Film size={18}/> Creator Studio</div><button className="studio-active"><BarChart3 size={16}/> Overview</button><button><Film size={16}/> My Movies</button><button><Download size={16}/> Upload Film</button><button><BarChart3 size={16}/> Analytics</button><button><Star size={16}/> Audience</button><button><List size={16}/> Payouts</button></aside><section><div className="dashboard-top"><div><div className="eyebrow">Creator workspace</div><h2>Good evening, Pacifique.</h2><p>Here is how your Rwandan stories are performing.</p></div><Button className="primary" onClick={() => toast('Upload flow will connect in the backend phase')}>+ Upload Film</Button></div><div className="dashboard-stats">{[['Total Views','124.8K','+18.4%'],['Subscribers','8,492','+9.2%'],['Movies','12','+2 this month'],['Revenue','$4.8K','+14.8%']].map(s=><div key={s[0]}><span>{s[0]}</span><strong>{s[1]}</strong><small>{s[2]}</small></div>)}</div><div className="dashboard-grid"><div className="dashboard-card"><h3>Performance</h3><div className="fake-chart"><i style={{height:'38%'}}/><i style={{height:'52%'}}/><i style={{height:'44%'}}/><i style={{height:'68%'}}/><i style={{height:'58%'}}/><i style={{height:'82%'}}/><i style={{height:'74%'}}/><i style={{height:'91%'}}/></div><div className="chart-labels"><span>Jan</span><span>Feb</span><span>Mar</span><span>Apr</span><span>May</span><span>Jun</span><span>Jul</span><span>Aug</span></div></div><div className="dashboard-card"><h3>Recent Content</h3>{movies.slice(0, 3).map(m=><div className="content-item" key={m.id}><img src={m.image} alt=""/><div><strong>{m.title}</strong><span>Published · 32K views</span></div><ChevronRight size={15}/></div>)}</div></div></section></div></div>}
