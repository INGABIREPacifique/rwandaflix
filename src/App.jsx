...
function MovieCard({ movie, onInfo, onPlay, onToggleList, inList }) {
  return (
    <article className="movie-card" tabIndex="0" aria-label={`${movie.title} details`} onClick={() => onInfo(movie)} onKeyDown={e => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); onInfo(movie) } }}>
...
function App() {
  ...
  const [list, setList] = useState(() => {
    try { return new Set(JSON.parse(localStorage.getItem('rwandaflix-my-list') || '[4,11]')) } catch { return new Set([4, 11]) }
  })
  const [notice, setNotice] = useState('')

  useEffect(() => {
    localStorage.setItem('rwandaflix-my-list', JSON.stringify([...list]))
  }, [list])
  ...
  useEffect(() => {
    document.body.style.overflow = selected || player || login || dashboard ? 'hidden' : ''
    ...
    <div className="app">
      <nav className="navbar">
        ...
        <div className={`nav-links ${mobileOpen ? 'open' : ''}`}>
          ...
          <button onClick={() => { setActiveGenre('All'); goTo('browse') }}>Genres</button>
