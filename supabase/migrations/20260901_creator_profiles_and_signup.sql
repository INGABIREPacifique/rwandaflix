...
-- Safe to rerun: existing policies are replaced instead of causing duplicate-policy errors.
-- Paste this entire file into Supabase SQL Editor and run it.

create table if not exists public.creator_profiles (
  id uuid primary key default gen_random_uuid(), user_id uuid not null unique references auth.users(id) on delete cascade,
  display_name text not null, bio text, avatar_url text, website_url text, verified boolean default false,
...
create table if not exists public.film_submissions (
  ...
  title text not null, description text, poster_url text, trailer_url text, video_url text, genre text,
  language text default 'Kinyarwanda', release_year integer, duration_minutes integer,
  status text not null default 'pending' check (status in ('pending','approved','rejected','published')),
  reviewer_notes text, created_at timestamptz default now(), updated_at timestamptz default now()
);

alter table public.creator_profiles enable row level security;
alter table public.film_submissions enable row level security;

-- Replace the six known policies so this migration is rerunnable.
...
create policy "Creators can submit films" on public.film_submissions for insert to authenticated with check (exists (select 1 from public.creator_profiles cp where cp.id = creator_id and cp.user_id = auth.uid()));
create policy "Creators can update pending submissions" on public.film_submissions for update to authenticated using (exists (select 1 from public.creator_profiles cp where cp.id = creator_id and cp.user_id = auth.uid()) and status = 'pending') with check (exists (select 1 from public.creator_profiles cp where cp.id = creator_id and cp.user_id = auth.uid()));
